#!/usr/bin/env python3
"""
ORBIT Cannes Batch Scrape-and-Verify Wrapper

Calls scripts/scrape-cannes.py for each year, then runs structural,
expectations, and landmark verification. Halts on anomaly by default.

Run:
  python3 scripts/scrape-cannes-batch.py --years 2024,2023,2022
  python3 scripts/scrape-cannes-batch.py --years 2024 --dry-run
  python3 scripts/scrape-cannes-batch.py --help
"""
...
SCRAPER = os.path.join(REPO_ROOT, "scripts", "scrape-cannes.py")
CATEGORIES_FILE = os.path.join(REPO_ROOT, "scripts", "scrape-cannes-categories.json")
EXPECTATIONS_FILE = os.path.join(REPO_ROOT, "data", "reference", "cannes-expectations.json")
TRUTH_FILE = os.path.join(REPO_ROOT, "data", "reference", "cannes-landmark-truth.json")
...
def resolve_expectation(category_entry, year):
    """Return (typical, tolerance) for this year, applying era_overrides if any."""
...
def phase_c_structural(rows, year, expectations):
    ...
    # Check 2: row count >= 8 (more allowed for ties)
    checks.append(("row_count_minimum",
                    len(rows) >= EXPECTED_CATEGORY_COUNT,
                    f"rows={len(rows)} (expected >= {EXPECTED_CATEGORY_COUNT})"))

    # Check 3: every expected category has at least 1 winner row
    cat_counts = Counter(cat_ids)
    zero_winner_cats = [c for c in expected_cats if cat_counts.get(c, 0) == 0]
    checks.append(("no_empty_categories",
                    len(zero_winner_cats) == 0,
                    f"zero_winner_cats={sorted(zero_winner_cats)}"
                    if zero_winner_cats else
                    "every expected category has >= 1 winner row"))

    # Check 4: all rows have result="won"
    non_won = sum(1 for r in rows if r.get("result") != "won")
    checks.append(("all_rows_won", non_won == 0,
                   f"non_won_rows={non_won}"))

    # Check 5: no empty film_title
...
def phase_d_expectations(rows, expectations, year):
    """Expectations check: per-category winner count vs typical+tolerance."""
    results = []
...
def phase_e_landmarks(rows, truth_entry):
    """Landmark verification (skipped if truth_entry is empty)."""
    results = []
...
def format_year_report(year, exit_code, rows, struct_checks,
    ...
    lines.append(f"## {year} Cannes Film Festival")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="ORBIT Cannes Batch Scrape-and-Verify Wrapper"
    )
    ...
    if bad:
        ...
        print(f"  Valid range: [{MIN_YEAR}, {MAX_YEAR}], excluding {sorted(SKIPPED_YEARS)}")
        sys.exit(1)
    ...
    for year in years:
        anomalies = []
        print(f"\n{'=' * 60}")
        print(f"[cannes batch {year}] Processing {year} Cannes Film Festival")
        print(f"{'=' * 60}")
        ...
        if rows is None:
            ...
            print(f"  [cannes batch {year}] FAIL — {load_err}")
        ...
        for cat_id, typical, tol, actual, verdict, _ in exp_results:
            if verdict == "MISSING":
                anomalies.append({"severity": "FAIL",
                                  "description": f"{cat_id}: 0 winner rows (expected ~{typical})"})
            elif verdict == "OVER":
        ...
        if verdict == "PASS":
            print(f"  [cannes batch {year}] PASS — {len(rows)} rows, "
                  f"{len(set(r['category_id'] for r in rows))} cats")
        elif verdict == "REVIEW":
            print(f"  [cannes batch {year}] REVIEW — {len(rows)} rows, "
                  f"{anomaly_count} anomalies (no critical failures)")
        else:
            print(f"  [cannes batch {year}] FAIL — {anomaly_count} anomalies "
