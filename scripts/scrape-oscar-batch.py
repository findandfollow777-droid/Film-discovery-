#!/usr/bin/env python3
"""
ORBIT Oscar Batch Scrape-and-Verify Wrapper

Calls scripts/scrape-oscar.py for each ceremony, then runs structural,
expectations, and landmark verification. Halts on anomaly by default.

Run:
  python3 scripts/scrape-oscar-batch.py --ceremonies 97,96
  python3 scripts/scrape-oscar-batch.py --range 97-72
  python3 scripts/scrape-oscar-batch.py --ceremonies 97,96 --dry-run
  python3 scripts/scrape-oscar-batch.py --help
"""
...
SCRAPER = os.path.join(REPO_ROOT, "scripts", "scrape-oscar.py")
CATEGORIES_FILE = os.path.join(REPO_ROOT, "scripts", "scrape-oscar-categories.json")
EXPECTATIONS_FILE = os.path.join(REPO_ROOT, "data", "reference", "oscar-expectations.json")
TRUTH_FILE = os.path.join(REPO_ROOT, "data", "reference", "oscar-landmark-truth.json")
...
# Flag that legitimately accompanies an unverified row (DLu had no candidate
# to cross-check against). Any other unverified row is treated as silent.
EXPECTED_UNVERIFIED_FLAG = "no_dlu_crosscheck_available"
...
def resolve_expectation(category_entry, ceremony_year):
    ...
    First matching override in the array wins. Falls back to top-level fields
...
def phase_c_structural(rows, ceremony_num, expectations):
    ...
    # Check 3: unverified rows must carry the legitimate "no_dlu_crosscheck_available"
    # flag — any other unverified row is silent and suspicious.
    silent_unverified = 0
    for r in rows:
        if r.get("verified_status") != "unverified":
            continue
        try:
            flags = json.loads(r.get("flags_json", "[]"))
        except (json.JSONDecodeError, TypeError):
            flags = []
        if EXPECTED_UNVERIFIED_FLAG not in flags:
            silent_unverified += 1
    checks.append(("no_silent_unverified", silent_unverified == 0,
                   f"silent_unverified={silent_unverified}"))

    # Informational: unverified count (not a fail condition for Oscar — DLu has
    # legitimate gaps for some categories, e.g. shorts).
    unverified = sum(1 for r in rows if r.get("verified_status") == "unverified")
    checks.append(("unverified_count_info", True, f"unverified={unverified} (informational)"))
...
def phase_e_landmarks(rows, truth_entry):
    ...
    for cat_id, lm in landmarks.items():
        ...
        # Winner subject (song) check — Oscar-specific for Best Original Song
        if "winner_subject" in lm:
            winners = [r for r in cat_rows if r["result"] == "won"]
            if not winners:
                results.append((cat_id, "winner_subject", False, "no winner row found"))
            else:
                w = winners[0]
                expected = normalize_for_compare(lm["winner_subject"])
                actual = normalize_for_compare(w.get("subject_title", ""))
                match = bool(actual) and (expected in actual or actual in expected)
                results.append((cat_id, "winner_subject", match,
                                f"expected='{lm['winner_subject']}' actual='{w.get('subject_title','')}'"))
...
def main():
    parser = argparse.ArgumentParser(
        description="ORBIT Oscar Batch Scrape-and-Verify Wrapper"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--ceremonies", type=str,
                       help=f"Comma-separated ceremony numbers (e.g. 97,96,95) "
                            f"in range [{MIN_CEREMONY},{MAX_CEREMONY}]")
    group.add_argument("--range", type=str,
                       help=f"Ceremony range high-low (e.g. 97-72)")
    ...
    # Validate scope
    out_of_scope = [c for c in ceremony_nums if c < MIN_CEREMONY or c > MAX_CEREMONY]
    if out_of_scope:
        print(f"ERROR: ceremonies {out_of_scope} outside scope [{MIN_CEREMONY}, {MAX_CEREMONY}]")
        sys.exit(1)
    ...
    for ceremony_num in ceremony_nums:
        ...
        print(f"\n{'=' * 60}")
        print(f"[oscar batch {ceremony_num}] Processing ceremony {ceremony_num} ({year})")
        print(f"{'=' * 60}")
        ...
        if rows is None:
            ...
            print(f"  [oscar batch {ceremony_num}] FAIL — {load_err}")
        ...
        if verdict == "PASS":
            print(f"  [oscar batch {ceremony_num}] PASS — {len(rows)} rows, "
                  f"{len(set(r['category_id'] for r in rows))} cats, 0 anomalies")
        elif verdict == "REVIEW":
            print(f"  [oscar batch {ceremony_num}] REVIEW — {len(rows)} rows, "
                  f"{anomaly_count} anomalies (no critical failures)")
        else:
            print(f"  [oscar batch {ceremony_num}] FAIL — {anomaly_count} anomalies "
