#!/usr/bin/env python3
"""
ORBIT Venice Batch Scrape-and-Verify Wrapper

Calls scripts/scrape-venice.py for each year, then runs structural,
expectations, and landmark verification. Halts on anomaly by default.

Run:
  python3 scripts/scrape-venice-batch.py --years 2024,2023,2022
  python3 scripts/scrape-venice-batch.py --years 2024 --dry-run
  python3 scripts/scrape-venice-batch.py --help
"""
...
SCRAPER = os.path.join(REPO_ROOT, "scripts", "scrape-venice.py")
CATEGORIES_FILE = os.path.join(REPO_ROOT, "scripts", "scrape-venice-categories.json")
EXPECTATIONS_FILE = os.path.join(REPO_ROOT, "data", "reference", "venice-expectations.json")
TRUTH_FILE = os.path.join(REPO_ROOT, "data", "reference", "venice-landmark-truth.json")
...
# Allow as few as 6 rows per ceremony — Venice's older ceremonies
# (notably cer 2003) have multiple confirmed Wikipedia article gaps
# (Special Jury Prize, Best Screenplay, Lion of the Future not all
# listed every year). Tighten this threshold as data quality improves.
...
def phase_c_structural(rows, year, expectations):
    """Structural verification mirroring Cannes wrapper architecture.

    Venice-specific allowances:
      - MIN_CATEGORIES_PER_CEREMONY=7 (not 9) because Mastroianni Award
        and Special Jury Prize have known gap years.
      - categories_present check still surfaces missing categories as
        FAIL — Phase 4 truth curation will whitelist genuine gap years.
    """
    checks = []
    ...
    # Venice has many legitimate gap years (Special Jury Prize not always
    # awarded; Mastroianni Award has gaps; some years' Wikipedia articles
    # are incomplete). categories_present is informational (always passes,
    # just surfaces the list); the row_count_minimum check below + Phase D
    # MISSING verdicts (REVIEW severity) provide the actual signal.
    ...
    # Treat zero_winner_cats as informational here — the categories_present
    # check above already raises FAIL on missing. zero_winner_cats provides
    # a slightly different angle (helpful for future era_override planning).
...
def format_year_report(year, exit_code, rows, struct_checks,
    ...
    lines.append(f"## {year} Venice International Film Festival")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="ORBIT Venice Batch Scrape-and-Verify Wrapper"
    )
    ...
    for year in years:
        anomalies = []
        print(f"\n{'=' * 60}")
        print(f"[venice batch {year}] Processing {year} Venice International Film Festival")
        print(f"{'=' * 60}")
        ...
        if rows is None:
            ...
            print(f"  [venice batch {year}] FAIL — {load_err}")
        ...
        if verdict == "PASS":
            print(f"  [venice batch {year}] PASS — {len(rows)} rows, "
                  f"{len(set(r['category_id'] for r in rows))} cats")
        elif verdict == "REVIEW":
            print(f"  [venice batch {year}] REVIEW — {len(rows)} rows, "
                  f"{anomaly_count} anomalies (no critical failures)")
        else:
            print(f"  [venice batch {year}] FAIL — {anomaly_count} anomalies "
