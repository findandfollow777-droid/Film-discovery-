#!/usr/bin/env python3
"""
ORBIT GG Batch Scrape-and-Verify Wrapper

Calls scripts/scrape-gg.py for each ceremony, then runs structural,
expectations, and landmark verification. Halts on anomaly by default.

Run:
  python3 scripts/scrape-gg-batch.py --ceremonies 83,82
  python3 scripts/scrape-gg-batch.py --range 83-57
  python3 scripts/scrape-gg-batch.py --ceremonies 83,82 --dry-run
  python3 scripts/scrape-gg-batch.py --help
"""
...
SCRAPER = os.path.join(REPO_ROOT, "scripts", "scrape-gg.py")
CATEGORIES_FILE = os.path.join(REPO_ROOT, "scripts", "scrape-gg-categories.json")
EXPECTATIONS_FILE = os.path.join(REPO_ROOT, "data", "reference", "gg-expectations.json")
TRUTH_FILE = os.path.join(REPO_ROOT, "data", "reference", "gg-landmark-truth.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="ORBIT GG Batch Scrape-and-Verify Wrapper"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--ceremonies", type=str,
                       help="Comma-separated ceremony numbers (e.g. 83,82,81)")
    group.add_argument("--range", type=str,
                       help="Ceremony range high-low (e.g. 83-57)")
    ...
    for ceremony_num in ceremony_nums:
        ...
        print(f"\n{'=' * 60}")
        print(f"[gg batch {ceremony_num}] Processing ceremony {ceremony_num} ({year})")
        print(f"{'=' * 60}")
        ...
        if rows is None:
            ...
            print(f"  [gg batch {ceremony_num}] FAIL — {load_err}")
        ...
        if verdict == "PASS":
            print(f"  [gg batch {ceremony_num}] PASS — {len(rows)} rows, "
                  f"{len(set(r['category_id'] for r in rows))} cats, 0 anomalies")
        elif verdict == "REVIEW":
            print(f"  [gg batch {ceremony_num}] REVIEW — {len(rows)} rows, "
                  f"{anomaly_count} anomalies (no critical failures)")
        else:
            print(f"  [gg batch {ceremony_num}] FAIL — {anomaly_count} anomalies "
