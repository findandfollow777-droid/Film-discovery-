...
let anchorCredits = null; // cached anchor credits (top-8 cast + director)

/* ============================================================
   6-FACTOR SIMILARITY MODEL — Ported from anchor.js 2026-04-30
   Replaces the legacy genre/vibe/era/rating model. Scores six
   factors: Genre, Cast DNA, Era, Director, Awards, Production
   Scale. Single weight set (chosen by anchor awards tier);
   ring assignment uses rank, not per-tier scoring.
   ============================================================ */

const BASE_WEIGHTS_DECORATED   = { genre: 0.43, castDna: 0.15, era: 0.10, director: 0.10, awards: 0.14, budgetBo: 0.08 };
const BASE_WEIGHTS_UNDECORATED = { genre: 0.50, castDna: 0.16, era: 0.11, director: 0.11, awards: 0.00, budgetBo: 0.12 };
...
/* ============================================================
   MODIFY SIGNAL — Added 2026-04-30
   User-tunable similarity weights. Pip 1–5 multiplies the base
   weight; getAdjustedWeights() normalises back to the original
   total. Ids match the BASE_WEIGHTS_* keys.
   ============================================================ */
const SIGNAL_FACTORS = [
  { id: 'genre',    label: 'Genre DNA',        desc: 'Tone, type & genre overlap',       icon: '◈', iconBg: 'var(--accent-cyan)',     iconOpacity: 0.10, color: 'cyan', default: 3 },
  { id: 'castDna',  label: 'Cast DNA',         desc: 'Shared actors across top billing', icon: '◉', iconBg: 'var(--prestige-purple)', iconOpacity: 0.10, color: 'cyan', default: 2 },
  { id: 'era',      label: 'Era',              desc: 'Proximity in release year',        icon: '◷', iconBg: 'var(--warning-orange)',  iconOpacity: 0.10, color: 'cyan', default: 2 },
  { id: 'director', label: 'Director',         desc: 'Same director match',              icon: '◎', iconBg: 'var(--success-green)',   iconOpacity: 0.10, color: 'cyan', default: 2 },
  { id: 'awards',   label: 'Awards Pedigree',  desc: 'Festival wins & nominations',      icon: '◆', iconBg: 'var(--accent-gold)',     iconOpacity: 0.08, color: 'gold', default: 1 },
  { id: 'budgetBo', label: 'Production Scale', desc: 'Budget tier match',                icon: '▣', iconBg: 'var(--muted-silver)',    iconOpacity: 0.08, color: 'cyan', default: 1 },
];

const PIP_MULTIPLIERS = { 1: 0.25, 2: 0.6, 3: 1.0, 4: 1.6, 5: 2.5 };

const signalState = {};
SIGNAL_FACTORS.forEach(f => signalState[f.id] = f.default);
...
function init() {
  ...
  // Modify Signal panel: render + wire (state defaults already applied)
  renderSignalPanel();
  syncChangedDot();
  wireSignalPanel();

  // Fetch anchor credits + details, then enrich films before scoring
  bootstrapWithCredits();
}

async function bootstrapWithCredits() {
  showCosmosLoader('READING THE STARS');
  try {
    const [credits, details] = await Promise.all([
      fetchFilmCredits(anchorMovie.id),
      fetchFilmDetails(anchorMovie.id)
    ]);
    anchorCredits = credits;
    anchorMovie._details = details;
    if (!anchorMovie.vote_count && details.vote_count) anchorMovie.vote_count = details.vote_count;
    if (!anchorMovie.vote_average && details.vote_average) anchorMovie.vote_average = details.vote_average;

    if (allMovies.length > 0) {
      await enrichFilmsWithCredits(allMovies);
    ...
    console.warn('[CONSTELLATION] Bootstrap failed:', e);
    hideCosmosLoader();
  }
}
...
// ============================================
// CREDITS + DETAILS FETCH (cached in sessionStorage)
// ============================================

async function fetchFilmCredits(movieId) {
...
async function enrichFilmsWithCredits(films) {
  ...
  for (let i = 0; i < films.length; i += batchSize) {
    ...
    const results = await Promise.all(batch.map(f =>
      Promise.all([fetchFilmCredits(f.id), fetchFilmDetails(f.id)])
    ));
...
function scoreBudgetBO(anchor, candidate) {
  ...
  let bonus = 0;
  if (tierScore > 0) {
    bonus = (perfBonus(anchor) + perfBonus(candidate)) / 2;
  }
...
function getAdjustedWeights(baseWeights) {
  ...
  for (const [key, base] of Object.entries(baseWeights)) {
    const pip = signalState[key] != null ? signalState[key] : 3;
    adjusted[key] = base * PIP_MULTIPLIERS[pip];
    total += adjusted[key];
    baseTotal += base;
  }
  if (total > 0 && baseTotal > 0) {
    for (const key of Object.keys(adjusted)) {
      adjusted[key] = (adjusted[key] / total) * baseTotal;
    }
  }
  return adjusted;
}

function calculateSimilarity(anchor, candidate) {
  ...
  const baseW = anchorIsRated ? BASE_WEIGHTS_DECORATED : BASE_WEIGHTS_UNDECORATED;
  const W = getAdjustedWeights(baseW);
  ...
  let castScore = 0;
  const aCast = anchorCredits ? anchorCredits.castIds : [];
  ...
  let directorScore = 0;
  const aDirId = anchorCredits ? anchorCredits.directorId : null;
  const cDirId = candidate._credits ? candidate._credits.directorId : null;
  if (aDirId && cDirId && aDirId === cDirId) directorScore = 100;
  ...
  // baseWeights sum to ~1.0, so total is on a 0–100 scale already.
...
function calculateSimilarities() {
  ...
  rankedMovies = allMovies.map(movie => ({
    movie,
    similarity: calculateSimilarity(anchorMovie, movie)
  }));
  rankedMovies.sort((a, b) => b.similarity - a.similarity);
}

// ============================================
// MODIFY SIGNAL — PANEL + LIVE RE-SCORE
// ============================================

function isSignalChanged() {
  return SIGNAL_FACTORS.some(f => signalState[f.id] !== f.default);
}

function syncChangedDot() {
  const dot = document.getElementById('signalChangedDot');
  if (!dot) return;
  dot.classList.toggle('visible', isSignalChanged());
}

function renderSignalPanel() {
  const host = document.getElementById('signalFactors');
  if (!host) return;
  let html = '';
  for (const f of SIGNAL_FACTORS) {
    const iconStyle = `background: color-mix(in srgb, ${f.iconBg} ${Math.round(f.iconOpacity * 100)}%, transparent); color: ${f.iconBg};`;
    let pipsHtml = '';
    for (let p = 1; p <= 5; p++) {
      const active = signalState[f.id] >= p ? ' active' : '';
      pipsHtml += `<button type="button" class="signal-pip${active}" data-factor="${f.id}" data-pip="${p}" data-color="${f.color}" aria-label="${f.label} weight ${p}"></button>`;
    }
    // Default tick between pip 3 and pip 4. With 17px pips and 6px gap,
    // pip centres are at 8.5, 31.5, 54.5, 77.5, 100.5. Midpoint 3↔4 = 66.
    pipsHtml += `<span class="signal-pip-default-tick" style="left: 66px;"></span>`;
    html += `<div class="signal-factor-row" data-factor-row="${f.id}">
      <div class="signal-factor-icon" style="${iconStyle}">${f.icon}</div>
      <div class="signal-factor-meta">
        <div class="signal-factor-name">${f.label}</div>
        <div class="signal-factor-desc">${f.desc}</div>
      </div>
      <div class="signal-factor-pips">${pipsHtml}</div>
    </div>`;
  }
  host.innerHTML = html;

  host.querySelectorAll('.signal-pip').forEach(pip => {
    pip.addEventListener('click', () => {
      const factorId = pip.getAttribute('data-factor');
      const pipVal = parseInt(pip.getAttribute('data-pip'), 10);
      if (!factorId || !pipVal) return;
      signalState[factorId] = pipVal;
      renderSignalPanel();
      syncChangedDot();
      rescoreCurrentRanking();
    });
  });
}

function rescoreCurrentRanking() {
  if (!anchorMovie || !allMovies || allMovies.length === 0) return;
  calculateSimilarities();
  renderOrbits();
}
...
function openSignalPanel() {
  ...
  const backdrop = document.getElementById('signalBackdrop');
  const chevron = document.getElementById('signalChevron');
...
function toggleSignalPanel() {
  const dropdown = document.getElementById('signalDropdown');
  if (!dropdown) return;
  if (dropdown.classList.contains('open')) closeSignalPanel();
  else openSignalPanel();
}
...
function wireSignalPanel() {
  ...
  if (btn) btn.addEventListener('click', (e) => { e.stopPropagation(); toggleSignalPanel(); });
  if (resetBtn) resetBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    resetSignalState();
    rescoreCurrentRanking();
  });
  if (backdrop) backdrop.addEventListener('click', closeSignalPanel);
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && dropdown?.classList.contains('open')) closeSignalPanel();
  });
}
...
async function reAnchor(movieId) {
  ...
  try {
    const [fullMovie, newAnchorCredits, newAnchorDetails] = await Promise.all([
    ...
    anchorCredits = newAnchorCredits;
    anchorMovie._details = newAnchorDetails;
    if (!anchorMovie.vote_count && newAnchorDetails.vote_count) anchorMovie.vote_count = newAnchorDetails.vote_count;
    if (!anchorMovie.vote_average && newAnchorDetails.vote_average) anchorMovie.vote_average = newAnchorDetails.vote_average;
    ...
    // Reset Modify Signal to defaults for the new anchor
    resetSignalState();
