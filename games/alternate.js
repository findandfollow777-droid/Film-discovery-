  ...
  document.querySelectorAll('.option-card').forEach(card => {
    ...
    if (actorId && actorName) {
      ...
      if (!card.querySelector('.clickable-overlay')) {
        ...
        overlay.addEventListener('click', (e) => {
          e.stopPropagation();
          window.location.href = `../pages/timeline.html?id=${actorId}&name=${encodeURIComponent(actorName)}`;
        });
  ...
  document.querySelectorAll('.result-bar').forEach(bar => {
    ...
    if (option) {
      ...
      bar.addEventListener('click', () => {
        window.location.href = `../pages/timeline.html?id=${option.id}&name=${encodeURIComponent(option.name)}`;
      });
    }
  });
}
