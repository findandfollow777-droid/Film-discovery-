...
// Merge TMDB-generated questions if the script is loaded (deduplicated)
function mergeQuestions(existing, generated) {
  const existingTexts = new Set(existing.map(q => q.q.toLowerCase()));
  // Also extract movie titles from existing questions to catch same-movie different-wording
  const existingTitles = new Set();
  for (const q of existing) {
    const match = q.q.match(/["""]([^"""]+)["""]/);
    if (match) existingTitles.add(match[1].toLowerCase());
  }
  return existing.concat(generated.filter(q => {
    if (existingTexts.has(q.q.toLowerCase())) return false;
    const match = q.q.match(/["""]([^"""]+)["""]/);
    if (match && existingTitles.has(match[1].toLowerCase())) return false;
    return true;
  }));
}

if (typeof TMDB_YEARS !== 'undefined') {
  QUESTIONS.years = mergeQuestions(QUESTIONS.years, TMDB_YEARS);
}
if (typeof TMDB_TAGLINES !== 'undefined') {
  QUESTIONS.taglines = mergeQuestions(QUESTIONS.taglines, TMDB_TAGLINES);
}

// Merge AI-generated questions if the script is loaded (deduplicated)
if (typeof AI_DIRECTORS !== 'undefined') {
  QUESTIONS.directors = mergeQuestions(QUESTIONS.directors, AI_DIRECTORS);
}
if (typeof AI_OSCARS !== 'undefined') {
  QUESTIONS.oscars = mergeQuestions(QUESTIONS.oscars, AI_OSCARS);
}
if (typeof AI_QUOTES !== 'undefined') {
  QUESTIONS.quotes = mergeQuestions(QUESTIONS.quotes, AI_QUOTES);
}
if (typeof AI_ACTORS !== 'undefined') {
  QUESTIONS.actors = mergeQuestions(QUESTIONS.actors, AI_ACTORS);
}
