  ...
  if (typeof initMovieCube === 'function') {
    initMovieCube({
      ...
      onAnchorClick: (movie) => {
        ...
        window.location.href = "../pages/anchor-point.html";
      }
    });
  }
...
function displayGuessResult(guess) {
  ...
  const awardBadges = getAwardBadgesHTML(guess.movieId, 24);
  ...
  if (awardBadges && window.renderAwardBadges) {
    window.renderAwardBadges(guessHistory.firstElementChild);
  }
}
...
function showResultSection(won) {
  ...
  if (target) {
    ...
    targetBadges.innerHTML = getAwardBadgesHTML(target.id, 32);
    if (window.renderAwardBadges) window.renderAwardBadges(targetBadges);
  }
