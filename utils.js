...
function _orbitGetPagePath() {
  ...
  const dir = parts.pop() || '';
  if (dir === 'games') return 'games/' + file;
  if (dir === 'pages') return 'pages/' + file;
  return file;
}
...
function getBackLabel(page) {
  ...
  const labels = {
    ...
    'anchor-point.html': 'Constellation View',
...
function _orbitBackHref(targetPath) {
  const currentPath = _orbitGetPagePath();
  const currentDir = currentPath.startsWith('games/') ? 'games' : currentPath.startsWith('pages/') ? 'pages' : 'root';
  const targetDir = targetPath.startsWith('games/') ? 'games' : targetPath.startsWith('pages/') ? 'pages' : 'root';
  const targetFile = targetPath.includes('/') ? targetPath.split('/').pop() : targetPath;

  if (currentDir === 'root') return targetPath;
  if (currentDir === targetDir) return targetFile;
  return '../' + targetPath;
}
