...
export async function getNebulaData(movieId) {
  ...
  // Fetch from data/nebula-data directory (resolve relative to this module's location)
  const nebulaBase = new URL('..', import.meta.url).href;
  try {
    const response = await fetch(new URL(`data/nebula-data/${movieId}.json`, nebulaBase));
